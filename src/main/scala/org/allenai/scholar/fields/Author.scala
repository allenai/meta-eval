...
case class Author(firstName: String, middleNames: Seq[String], lastName: String) {
  override def toString = {
    (firstName +: middleNames :+ lastName).map(_.removePunctuation).mkString(" ").collapseWhitespace
  }
  def normalized = Author(firstName.normalized, middleNames.map(_.normalized), lastName.normalized)

  def lastNameOnly = Author("", Seq(), lastName)

  def lastNameNormalized = lastName.removePunctuation.normalized
